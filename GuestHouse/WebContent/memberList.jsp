...
<body>
<%
	List<LoginDTO> memberList = (List<LoginDTO>)request.getAttribute("memberList");
%>
...
<%
		for(int i=0; i<memberList.size(); i++){
			LoginDTO dto = memberList.get(i);
%>
	<tbody>
		<tr>
			<td><%=i+1%></td>
			<td><a href="memberView.login?id=<%=dto.getId()%>"><%=dto.getId()%></a></td>
			<td><%=dto.getCompetence()%></td>
			<td><a href="memberUpdateForm.login?id=<%=dto.getId()%>">정보수정</a></td>
			<td><a href="memberDelete.login?id=<%=dto.getId()%>">계정삭제</a></td>
		</tr>
	</tbody>
<%
		}
